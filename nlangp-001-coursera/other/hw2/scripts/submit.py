...
def submit():   
  ...
  (login, password) = ('sasha.rudan@gmail.com', '2sx6vGEzcf')#loginPrompt()
  if not login:
...
partIds = ['parser-part1', 'parser-part2', 'parser-part3']                        
# used to generate readable run-time information for students
partFriendlyNames = ['Parser Setup', 'CKY Decoder', 'Markovization'] 
          
def output(partIdx):
  """Uses the student code to compute the output for test cases."""
  if partIdx == 0:
    file = "parse_train.counts.out"
  else:
    file = "parse_test.p%d.out"%(partIdx + 1)
  try:
